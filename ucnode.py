...
##                                                                           ##
## Simulator where the nodes use CoROA.                                      ##
##                                                                           ##
##  TODO:                                                                    ##
##  * Check if a node that has just become head is able to reach its next    ##
##    hop                                                                    ##
...
from ucmessages import MessageGenerator as MG, BASIC_TTL
from ucmessages import BROADCAST_ADDR, Message, UOARFlags, UOARTypes
...
class UCNode:
    ...
    def __init__(self, addr, x, y, depth, energy, clock, verbose = False):
        ...
        self.rcvdRAfromMember = False
        self.roundsToRequest = INFINITY
        self.maybeNextHop    = None
        self.maybeHopsToSink = INFINITY
        # for retransmissions
        self.numRetries = 0
    ...
    def execute(self, maxTime, isNewSlot):
        ...
        if isNewSlot: # new round
            ...
            if self.round == self.nextAgg and self.state is UOARState.CLUSTER_HEAD:
                self.aggregate_data()           
            ...
            if self.status is UOARStatus.READY:
                ...
                if self.state is UOARState.CLUSTER_MEMBER:
                    self.status = UOARStatus.MEMBER_WAIT
                else:
                    self.status = UOARStatus.HEAD_WAIT
                    
            elif self.status is UOARStatus.MEMBER_WAIT: 
                if self.stopWaiting:
                    if self.isSink:
                        self.status = UOARStatus.HEAD_WAIT
                    else:
                    ...
                    if self.roundsToRequest is 0:
                        self.status = UOARStatus.EXCHANGE
                        self.roundsToRequest = INFINITY
            
            elif self.status is UOARStatus.EXCHANGE and \
                self.hopsToSink is not INFINITY:
                self.status = UOARStatus.HEAD_WAIT
        ...
        if self.state is UOARState.INITIAL:
            ...
            if self.status is UOARStatus.READY:
                ...
                if self.roundsToRequest < 0:
                    print('Error: negative rounds to request')
                if self.roundsToRequest == INFINITY and \
            ...
            elif self.status is UOARStatus.EXCHANGE:
                ...
                if self.nextHop in self.cheadList.keys():
                    ...
                    time, energy = Tools.estimate_transmission(msg)
                    self.energy -= energy
                    time = maxTime - self.opticalAckTime
                ...
                if self.hopsToSink is not INFINITY:
                    if self.maybeNextHop is not None:
                        ...
                        if not False in self.cheadList.values():
                            ...
                            print('Node ' + str(self.addr) + ' can stop')
                        else:
        ...
        if len(self.outbox) is not 0:
            ...
            if (nextMsg.flags & 0x0f) is UOARTypes.COMMON_DATA:
                ...
                if self.state is UOARState.CLUSTER_HEAD: 
                    ...
                    nextMsg.flags |= UOARFlags.HEAD_SRC 
                else:
                    nextMsg.flags &= ~UOARFlags.ACOUSTIC 
                    nextMsg.flags &= ~UOARFlags.HEAD_SRC 
    ...
    def handle_message(self, msg):
        ...
        if msgType is UOARTypes.COMMON_DATA:
                    ...
                    if self.state is UOARState.CLUSTER_MEMBER:
                        ...
                        if fromHead == 1:
                            ...
                            if innerMsg.ctime == self.lastAgg['ctime']:
                                self.incWait = (self.round - self.lastAgg['round']) + 1
            ...
            if msg.src in self.oneighbors:
                if self.status is UOARStatus.DISCOVERING:
                    self.nextHop = msg.src
                elif self.state is UOARState.CLUSTER_MEMBER and \
                     nodeIsHead and msg.src is not self.nextHop:
                    self.nextHop = msg.src
                elif (self.status is UOARStatus.HEAD_WAIT and \
            ...
            if (self.status is UOARStatus.MEMBER_WAIT or \
               self.status is UOARStatus.ELECTING or \
               self.status is UOARStatus.EXCHANGE):
                if self.nextHop is msg.src:
                    ...
                    #
                    if not self.rcvdRAfromMember:
                        self.rcvdRAfromMember = True
                        self.roundsToRequest  = self.MAX_WAIT_ROUNDS
                        self.maybeNextHop     = msg.src
                        self.maybeHopsToSink  = nodeHops

                    elif self.rcvdRAfromMember and nodeIsHead:
                        self.maybeNextHop    = msg.src      
                        self.maybeHopsToSink = nodeHops          
        ...
        elif msgType is UOARTypes.REQ_EXCHANGE:
            if self.verbose:
                print('handling exchange request from node ' + str(msg.src))

            if self.state is UOARState.CLUSTER_HEAD:
                if self.status is UOARStatus.HEAD_WAIT and  \
                   self.hopsToSink == INFINITY and self.maybeNextHop is None:
                    # Is not in route yet
                    self.maybeNextHop = msg.src
                else:
            ...
            if self.status is UOARStatus.EXCHANGE:
                if nodeCanChange:
                    ...
                    if self.status is UOARStatus.EXCHANGE and \
